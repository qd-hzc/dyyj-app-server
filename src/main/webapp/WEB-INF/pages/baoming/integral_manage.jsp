    ...
    var columns = [//列定义
        ...
        },
        {
            ...
            "render": function (data, type, row) {
                var id = row.id;
                if (row.status == 3) {
                    return "<input data-value='" + id + "'checked='checked' class='ace ace-switch ace-switch-9 skip-validation' type='checkbox'><span class='lbl'></span>";
                } else if (row.status == 4) {
                    return "<input data-value='" + id + "' class='ace ace-switch ace-switch-9 skip-validation' type='checkbox'><span class='lbl'></span>";
                }
//                        return "<input type='button' class='btn btn-primary btn-xs' value='已缴费' onclick='openAbstract(" + row.id + ");'>";
            }
        }
    ];
    ...
    new DefaultDataTable(Routers.accounting.enrollManage.searchList, columns, '#example', '#enrollSearch');
